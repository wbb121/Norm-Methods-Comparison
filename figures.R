...
ibd_pcoa_scatter <- ibd_pcoa_scatter+
  geom_text(data=ibd_study_labels,mapping=aes(x=0.35,y=-0.4,label=r2),parse=TRUE,inherit.aes=FALSE,hjust=0,size=4)+
  geom_text(data=ibd_study_labels,mapping=aes(x=0.35,y=-0.45,label=pvalue),parse=TRUE,inherit.aes=FALSE,hjust=0,size=4)
ibd_pcoa_scatter

### heatmaps of bray curtis distance between different studies
ibd_studies <- unique(ibd_meta$study_name_simplified)   # ibd studies
...
#=====================================================================================================================#
### heatmap for simulation results ###
#=====================================================================================================================#
### data 
norm_methods <- readRDS("data/norm_methods.rds")
sim_pred_summ <- readRDS("data/sim_pred_summ.rds")
...
### heatmap
heatmap.func <- function(df_data,ed,title){
  # heatmaps
  df_data <- df_data[df_data$ed==ed,]
  df_data <- df_data[df_data$norm_method%in%norm_methods$norm_method,]
  df_data$alpha <- factor(paste0("ep=",df_data$alpha))
  df_data$average_auc <- round(df_data$average_auc,3)
  heatmaps <- ggplot(df_data, aes(x=alpha, y=annotated_method, fill=average_auc))+ 
    geom_tile() +
    geom_text(aes(x=alpha, y=annotated_method, label=average_auc), color="black", size=4)+
...
p1 <- heatmap.func(df_data=sim_pred_summ,ed=1.02,title=expression(paste(bold("A")," ed = 1.02")))
p2 <- heatmap.func(df_data=sim_pred_summ,ed=1.04,title=expression(paste(bold("B")," ed = 1.04")))+theme(axis.text.y=element_blank())
p3 <- heatmap.func(df_data=sim_pred_summ,ed=1.06,title=expression(paste(bold("C")," ed = 1.06")))+theme(axis.text.y=element_blank())
p1 %>% insert_left(margins,width=0.05) %>% 
  insert_right(p2, width=1) %>%
  insert_right(p3,width=1)
ggsave("sim_pred_heatmap.jpg",width=18,height=6,dpi=300)


#=====================================================================================================================#
### prediction results for CRC datasets ###
#=====================================================================================================================#
### data 
